 ...
 */
async function addDependenciesRecursive(depsIndex, projectRootPath, exclusions, inclusions, rootPath) {
  ...
  do {
    ...
    for (let i = startOfSublist; i <= endOfSublist; i++) {
      ...
      debug(`    + ${currentParentPath} -> ${element.fullName}@${element.installedVersion}`);
      ...
      if(!pathIncludes(currentParentPath, element)) {
        ...
        if (fs.existsSync(elementPackageJsonPath)) {
          ...
          const selectedDependencies = getDependencies(packageJson, exclusions, inclusions, element.path);
          if (selectedDependencies.length > 0) {
            // mark elements in 'requires' that would create a dependency loop
            element.requires = selectedDependencies.sort(util.alphaSort);
            element.requires.forEach(requiredPackage => {
            ...
            const selectedDependenciesComplete = await Promise.all(
              selectedDependencies.map(async (element) => {
                const localDataForPackages = await addLocalPackageData(element, elementPackageJsonPath);
                const packagesData = await addPackageDataFromRepository(localDataForPackages);
                const basicFields = { fullName: element.fullName, alias: element.alias, path: element.path };
