 ...
 * @returns array with fieldsToRemove and array with fieldsToAdd
 */
 ...
 * @return {object[]} dedupedSortedList converted to a tree
 */
  ...
  for (let i = 0; i <= endOfList; i++) {
    ...
    if ((subDeps !== undefined) && Array.isArray(subDeps)) {
      ...
      subDeps.forEach(dependency => {
        const subId = `${dependency.fullName}@${dependency.installedVersion}`;
